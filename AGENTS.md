...
- A **module's** file-level block opens `/** @fileoverview `. The tag is what distinguishes a description of the file from a description of whatever declaration follows it, and it needs `/**` rather than `/*` because a plain block carries no JSDoc tags. **Test files use a plain `/* */` block**: nothing imports them, so there is no declaration for the tag to disambiguate from and no tooling reading it
