...
document.addEventListener('DOMContentLoaded', () => {
  ...
  crInput.addEventListener('click', ({ target }) => changeCR(target, options));
  crInput.addEventListener('change', ({ target }) => changeCR(target, options));
  // Change complex imaginary part
  const ciInput = document.getElementById('ciInput');
  ciInput.addEventListener('click', ({ target }) => changeCI(target, options));
  ciInput.addEventListener('change', ({ target }) => changeCI(target, options));
  // Change x-offset
...
document.addEventListener('keydown', ({ key }) => {
  /**
   * wasd to move, q and e to zoom in/out respectively,
   * z and x to increase/decrease iterations respectively,
   * r to restart
   */
  switch (key) {
    case 'w':
