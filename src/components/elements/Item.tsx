...
const itemVariants = cva(
  "group/item flex w-full flex-wrap items-center rounded-md border text-sm transition-colors duration-100 outline-none focus-visible:border-ring in-data-[state=focus]:border-ring focus-visible:ring-[3px] in-data-[state=focus]:ring-[3px] focus-visible:ring-ring/50 in-data-[state=focus]:ring-ring/50 [a]:transition-colors [a]:hover:bg-muted",
  {
    variants: {
      variant: {
...
type ItemProps = useRender.ComponentProps<"div"> &
  VariantProps<typeof itemVariants> & {
    /** When set, the Item renders as an `<a>` and the whole row becomes clickable.
     * Hover styling (`[a]:hover:bg-muted`) lights up automatically. */
    href?: string;
  };
