...
import { burningShip, mandelbrotSet, multibrotSet, tricorn } from "@/lib/fractals/algorithms";
import type { EscapeParams } from "@/lib/fractals/types";

const defaultOptions: EscapeParams = {
  maxIterations: 100,
  offsetX: 0,
  offsetY: 0,
  zoom: 1,
...
const juliaOptions: EscapeParams = {
  maxIterations: 100,
  offsetX: 0,
  offsetY: 0,
  zoom: 1,
...
describe("mandelbrotSet", () => {
  ...
  `(
    "returns $expected for canvas point ($x,$y) at $width x $height",
    ({ x, y, width, height, options, expected }) => {
      expect(mandelbrotSet(x, y, width, height, options)).toStrictEqual(expected);
    },
  );
});
...
describe("burningShip", () => {
  ...
  `(
    "returns $expected for canvas point ($x,$y) at $width x $height",
    ({ x, y, width, height, options, expected }) => {
      expect(burningShip(x, y, width, height, options)).toStrictEqual(expected);
    },
  );
});
...
describe("tricorn", () => {
  ...
  `(
    "returns $expected for canvas point ($x,$y) at $width x $height",
    ({ x, y, width, height, options, expected }) => {
      expect(tricorn(x, y, width, height, options)).toStrictEqual(expected);
    },
  );
});
...
describe("multibrotSet", () => {
  ...
  `(
    "returns $expected for canvas point ($x,$y) at $width x $height",
    ({ x, y, width, height, options, expected }) => {
      expect(multibrotSet(x, y, width, height, options)).toStrictEqual(expected);
    },
  );
});
...
describe("Julia Set options for each", () => {
  test.each`
    x      | y      | width  | height | options         | fractal          | expected
  ...
  `(
    "returns $expected for canvas point ($x,$y) with $fractal",
    ({ x, y, width, height, options, fractal, expected }) => {
      expect(fractal(x, y, width, height, options)).toStrictEqual(expected);
    },
  );
});
